...
function toCamelCase(name, uppercase) {
  const startIndex = uppercase ? 0 : 1;
  const lowerName = name.toLowerCase();
  const nameArr = lowerName.split('-');
  for (let i = startIndex; i < nameArr.length; i++) {
...
function createLinkTag(rel, href) {
  ...
  const newRel = rel.trim();
  const newHref = href.trim();
  const reExp = /[\s\t\n\r]/;
  console.log(newRel, newHref);

  const relNeedsTest = reExp.test(newRel);
  const hrefNeedsTest = reExp.test(newHref);
  const finalRel = relNeedsTest ? `"${newRel}"` : `${newRel.replace(/'"/, '')}`;
  const finalHref = hrefNeedsTest ? `"${newHref}"` : `${newHref.replace(/'"/, '')}`;
  console.log(finalRel, finalHref);

  return `<link rel=${finalRel} href=${finalHref}>`;
}
...
function parseDateString(value) {
  ...
  const errorMsgMos = 'Month must be between 01 and 12';
  const errorMsgDay = 'Day must be between 01 and 31';
  ...
  if (format1.test(value)) {
    // Check for format 1 match and split into array
    const match = format1.exec(value);
    year = parseInt(match[1]);
    month = parseInt(match[2]) - 1;
    day = parseInt(match[3]);
  } else if (format2.test(value)) {
    // Check for format 2 match and split into array
    const match = format2.exec(value);
  ...
  // Validate month
  if (month < 0 || month > 11) {
    throw new Error(errorMsgMos);
  }

  // Validate day
  if (day < 1 || day > 31) {
    throw new Error(errorMsgDay);
  }
...
console.log(toDateString(parseDateString('2021/01/29'), 'YYYY/MM/DD')); // "2021/01/29"
console.log(toDateString(parseDateString('2021/01/29'), 'DD/MM/YYYY')); // "29/01/2021"
console.log(toDateString(parseDateString('29/01/2021'), 'MM/DD/YYYY')); // "01/29/2021"
...
function normalizeTime(value) {
  // Regular expressions
  const format12Hour = /^(\d{1,2}):(\d{2}):(\d{2})\s?(AM|PM)$/i;
  const format24Hour = /^(\d{1,2}):(\d{2}):(\d{2})$/;
  let hours;
  let minutes;
  let seconds;

  if (format12Hour.test(value)) {
    // Check match for 12-hour format
    const match = format12Hour.exec(value);
    hours = parseInt(match[1]);
    minutes = parseInt(match[2]);
    seconds = parseInt(match[3]);
    const period = match[4].toUpperCase();

    // Validating hour, minutes and seconds as per instructions
    if (hours < 1 || hours > 12 || minutes < 0 || minutes > 59 || seconds < 0 || seconds > 59) {
      return null;
    }

    // Adjust time for pm and am accordingly for 24 format
    if (period === 'PM' && hours !== 12) {
      hours += 12;
    } else if (period === 'AM' && hours === 12) {
      hours = 0;
    }
  } else if (format24Hour.test(value)) {
    // Check match for 24-hour format
    const match = format24Hour.exec(value);
    hours = parseInt(match[1]);
    minutes = parseInt(match[2]);
    seconds = parseInt(match[3]);
...
function formatTimes(...values) {
  const timesList = values.map((value) => normalizeTime(value)).filter((time) => time !== null);
...
function mimeTypeFromFilename(filename) {
  ...
  const extension = filenameArr[filenameArr.length - 1].toLowerCase().trim();
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  // Validate that resourceType and resourceId are provided and non-empty
  if (!resourceType || (typeof resourceType !== 'string' && typeof resourceType !== 'number')) {
  ...
  // Convert resourceId to string if it's not already
  resourceId = String(resourceId);

  // Start building the endpoint
  let endpoint = `/${resourceType}/${resourceId}`;

  // If extraData is provided, validate and encode it
